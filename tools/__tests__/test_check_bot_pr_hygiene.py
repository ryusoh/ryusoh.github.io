...
def test_bot_suppressions_addition_flagged(repo: Path) -> None:
    _write_and_commit(
        ...
        '{"js/core.js": {"complexity": {"count": 1}}}\n',
        "refactor(core): add suppression",
    )
    violations = find_violations(repo, "master")
...
def test_bot_suppressions_increase_flagged(repo: Path) -> None:
    ...
    _write_and_commit(
        repo,
        "eslint-suppressions.json",
        '{"js/core.js": {"complexity": {"count": 2}}}\n',
        "refactor(core): increase count",
        bot=True,
    )
    violations = find_violations(repo, "master")

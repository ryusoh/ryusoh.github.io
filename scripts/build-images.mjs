...
async function processImages() {
    ...
    for (const page of pages) {
        ...
        for (const file of files) {
            ...
            console.log(
                `[${count}/74] ${file}: orig ${(origStat.size / 1024).toFixed(1)}KB -> avif full ${(avifFullStat.size / 1024).toFixed(1)}KB, 768w ${(avif768Stat.size / 1024).toFixed(1)}KB`
            );
        }
    }
    ...
    console.log(
        `AVIF full (desktop):  ${(totalAvifFullBytes / (1024 * 1024)).toFixed(2)} MB (${((1 - totalAvifFullBytes / totalOriginalBytes) * 100).toFixed(1)}% reduction)`
    );
    console.log(
        `AVIF 768w (mobile):   ${(totalAvif768Bytes / (1024 * 1024)).toFixed(2)} MB (${((1 - totalAvif768Bytes / totalOriginalBytes) * 100).toFixed(1)}% reduction)`
    );
    ...
    for (const page of pages) {
        ...
        const pictureOrImgRegex =
            /<picture>\s*<source[^>]+>\s*<source[^>]+>\s*(<img\s+[^>]*?src="\/assets\/img\/(p\d)\/([^"]+?\.(jpe?g|JPG))"([^>]*?)\/?>)\s*<\/picture>|<img\s+([^>]*?src="\/assets\/img\/(p\d)\/([^"]+?\.(jpe?g|JPG))"([^>]*?))\s*\/?>/gs;

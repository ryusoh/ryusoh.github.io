...
**Learning:** Adding `aria-keyshortcuts` ensures screen reader users are aware of keyboard navigation commands (like Escape or Arrow keys). However, sighted keyboard or mouse users have no visual indication that these shortcuts exist in minimalist interfaces without explicit shortcut hints.
**Action:** Always pair hidden keyboard shortcuts (e.g., `aria-keyshortcuts`) on interactive elements with native visual tooltips (e.g., `title` attributes) indicating the shortcut. This significantly improves discoverability for all users without breaking minimalist design constraints.

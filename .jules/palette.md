...
## 2025-02-28 - Unified High-Contrast Interactive States
**Learning:** This specific repository uses a custom dark theme design where default link hovers and `:focus-visible` outlines were fragmented (white text for hovers, dim red `#d9534f` for focus). Dim red on dark backgrounds creates insufficient contrast for focus states, leading to poor keyboard navigation accessibility.
**Action:** Unify all interactive states (custom cursor inner dot, link hovers, and focus-visible rings) to use the project's high-contrast neon lime green (`#D2FF00`). This ensures a11y signaling is visually distinct, cohesive, and easily recognizable across both mouse and keyboard interactions.

...
## 2024-05-23 - [Accessibility: Discoverability of Keyboard Shortcuts]

**Learning:** When using icon-only buttons with keyboard shortcuts (`aria-keyshortcuts`), the shortcuts remain undiscoverable to sighted users who prefer mouse or touch interfaces unless explicitly told.
**Action:** Always pair hidden keyboard shortcuts (e.g., `aria-keyshortcuts`) on interactive elements with native visual tooltips (e.g., `title` attributes). This ensures discoverability for sighted users without cluttering a minimalist visual interface or introducing complex custom tooltip logic.

...
## 2024-05-18 - [Accessibility: Discoverability of aria-keyshortcuts]

**Learning:** Hidden keyboard shortcuts defined via `aria-keyshortcuts` on interactive elements remain undiscoverable to sighted keyboard users who do not rely on screen readers. To ensure full accessibility and usability for all users, hidden keyboard shortcuts must be paired with visible tooltips.
**Action:** When implementing hidden keyboard shortcuts (e.g., `Escape` or `ArrowRight`) on interactive elements like navigation buttons, always pair them with native visual tooltips using the `title` attribute (e.g., `title="Back to home (Esc)"`) to ensure discoverability for sighted users.

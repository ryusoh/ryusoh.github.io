...
## 2026-11-23 - [UX/Accessibility: Shortcut Discoverability]

**Learning:** Hidden keyboard shortcuts (like `aria-keyshortcuts="Escape"` or `aria-keyshortcuts="ArrowRight"`) on interactive elements provide great utility for advanced and non-visual users, but sighted mouse users often remain unaware of them unless they explicitly read the source or help documentation.
**Action:** Always pair hidden keyboard shortcuts with native visual tooltips (`title` attributes) on the associated interactive elements (e.g., `title="Back to home (Esc)"`). This bridges the gap between keyboard accessibility and visual discoverability without cluttering the main UI layout.

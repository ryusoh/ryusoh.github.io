...
## 2024-05-18 - [Accessibility: Discoverability of Keyboard Shortcuts]

**Learning:** When hidden keyboard shortcuts are provided (e.g., `aria-keyshortcuts="Escape"` on navigation links), sighted users might not know they exist, particularly when minimal UI lacks explicit instructions.
**Action:** Always pair hidden keyboard shortcuts (`aria-keyshortcuts`) on interactive elements with native visual tooltips (using the `title` attribute) to ensure the shortcuts are discoverable by sighted users while remaining accessible to screen reader users.

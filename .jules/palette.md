...
## 2024-05-18 - [Accessibility: Keyboard Shortcut Discoverability]

**Learning:** When adding keyboard shortcuts to interactive elements (e.g., using `aria-keyshortcuts`), it's essential to pair them with native visual tooltips (`title` attributes). Without visual indications, sighted users will remain unaware of these shortcuts, missing out on improved keyboard navigation and efficiency.
**Action:** Always ensure that hidden keyboard shortcuts like `aria-keyshortcuts` are accompanied by a native visual `title` attribute, specifically detailing the shortcut key (e.g., `title="Back to home (Esc)"`).

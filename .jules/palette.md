...
**Learning:** While `aria-keyshortcuts` exposes keyboard shortcuts to assistive technologies, these shortcuts remain completely invisible to sighted users, including keyboard-only users who don't use screen readers.
**Action:** Always pair `aria-keyshortcuts` attributes on interactive elements with native `title` attributes (or custom tooltips) that explicitly state the shortcut (e.g., `title="Back to home (Escape)"`) to ensure full discoverability for all user types.

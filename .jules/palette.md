...
## 2024-05-13 - [Accessibility/UX: Keyboard Shortcut Discoverability]

**Learning:** Providing `aria-keyshortcuts` on interactive elements is excellent for programmatic accessibility, but these shortcuts remain invisible to sighted users, leading to poor discoverability. Users might hover over a navigation button (like a "Back" or "Next" arrow) but never realize they could simply press `Escape` or `ArrowRight`.
**Action:** To ensure keyboard accessibility and visual discoverability, always pair hidden keyboard shortcuts (e.g., `aria-keyshortcuts`) on interactive elements with native visual tooltips by using the `title` attribute (e.g., `title="Back to home (Escape)"`).
